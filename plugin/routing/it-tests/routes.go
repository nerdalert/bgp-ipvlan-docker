...
func addLinuxRoute(neighborNetwork *net.IPNet, nextHop net.IP, netIface string) error {
...
func OriginateBgpRoute(localPrefix string) error {
	log.Infof("Adding this hosts container network [ %s ] into the BGP domain", localPrefix)
	_, stderr, err := gobgp(bgpCmd, global, rib, bgpAdd, localPrefix, addrFamily, ipv4)
	if err != nil {
...
// AddHostRoute adds a route to a device using netlink into the Linux default namespace.
func delLinuxRoute(neighborNetwork *net.IPNet, nextHop net.IP, netIface string) error {
	iface, err := netlink.LinkByName(netIface)
	if err != nil {
		return err
	}
