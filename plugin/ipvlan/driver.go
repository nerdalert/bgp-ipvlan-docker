...
func New(version string, ctx *cli.Context) (Driver, error) {
	...
	case ipVlanL3:
		...
		// Initialize BGP monitoring
		go routing.InitBGPMonitoring(ipVlanEthIface)
...
func (driver *driver) createNetwork(w http.ResponseWriter, r *http.Request) {
	...
	log.Debugf("Received a create network from libnetwork [ %s ]", containerCidr.String())
	...
	if ipVlanMode == ipVlanL3 {
		...
		if err != nil {
			log.Debugf("Error occoured finding the linux link [ %s ] from netlink: %s", ipVlanEthIface, err)
		}
		...
		err = routing.OriginateBgpRoute(containerCidr)
		if err != nil {
